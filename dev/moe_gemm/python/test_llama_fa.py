...
try:
    BLOCK_CONFIG = json.load(open("block_config.json"))
except:
    BLOCK_CONFIG = {}
...
class FastLinear(nn.Module):
    ...
    def forward(self, input: torch.Tensor, block_x=None, block_y=None) -> torch.Tensor:
        if input.shape[0] == 1 and block_x is None and block_y is None:
            block_x, block_y = BLOCK_CONFIG.get(
                f"{input.shape[0]},{input.shape[1]},{self.weight.shape[0]},{self.weight.shape[1]}",
                [32, 8],
            )

        if input.shape[0] > 1 or (block_x == 0 or block_y == 0):
            return moe_matmul.moe_gemm(input, self.weight.T)
        else:
            return moe_matmul.moe_gemm(
                self.weight, input.view(-1, 1), block_x, block_y
            ).view(1, -1)
...
class FlashLlamaAttention(torch.nn.Module):
    def __init__(
        ...
        # q,k,v,o and rotary
        self.query_key_value = weights.load_multi_linear(
            config, [f"{prefix}.q_proj", f"{prefix}.k_proj", f"{prefix}.v_proj"]
        )
    ...
    def forward(
        ...
        cache_ops.reshape_and_cache(kv[:, 0], kv[:, 1], kv_cache[0], kv_cache[1], slots)
...
class LlamaMLP(nn.Module):
    def __init__(self, prefix, config, weights):
        ...
        self.gate_up_proj = weights.load_multi_linear(
            config, [f"{prefix}.gate_proj", f"{prefix}.up_proj"]
        )
        self.down_proj = FastLinear.load(
            config, f"{prefix}.down_proj", weights, bias=False
        )
...
class FlashLlamaModel(torch.nn.Module):
    def __init__(self, config, weights):
        ...
        self.embed_tokens = nn.Embedding.from_pretrained(
            F.pad(embeddings, (0, 0, 0, 1)), padding_idx=config.pad_token_id
        )
...
class FlashLlamaForCausalLM(torch.nn.Module):
    def __init__(self, config, weights):
        ...
        self.lm_head = FastLinear.load(
            config, prefix="lm_head", weights=weights, bias=False
        )
...
class CacheManager:
    ...
    def allocate(self, blocks, max_blocks, needed_blocks_slots):
        ...
        return (
            block_tables,
            block_tables_tensor.to(self.device),
            torch.concat(slots).to(self.device),
        )
...
def generate(tokenizer, model, prompt, max_new_tokens=10):
    ...
    def warmup():
        ...
        warmup_inputs = {
            "input_ids": torch.arange(
                1, input_length + 1, dtype=torch.int64, device=device
            ).repeat(bs),
            "position_ids": torch.arange(
                0, input_length, dtype=torch.int32, device=device
            ).repeat(bs),
            "cu_seqlen_prefill": torch.tensor(
                [i * input_length for i in range(bs + 1)],
                dtype=torch.int32,
                device=device,
            ),
            "block_tables": torch.arange(
                0, blocks, dtype=torch.int32, device=device
            ).split(blocks // bs),
            "slots": torch.arange(0, 4144, dtype=torch.int32, device=device),
            "input_lengths": torch.tensor(
                [input_length] * 4, dtype=torch.int32, device=device
            ),
            "max_s": 1024,
    ...
    _, block_tables_tensor, slots = CACHE_MANAGER.allocate(
        needed_blocks, needed_blocks, needed_blocks_slots
    )
    # forward循环
    loops = 10
    for loop in range(loops):
        ...
        for step in range(max_new_tokens):
            if step == 0:
                ...
                inputs = {
                    ...
                    "position_ids": torch.arange(
                        0, len(input_ids), dtype=torch.int32, device=device
                    ),
                    "cu_seqlen_prefill": torch.tensor(
                        [0, len(input_ids)], dtype=torch.int32, device=device
                    ),
                    ...
                    "input_lengths": torch.tensor(
                        [len(input_ids)], dtype=torch.int32, device=device
                    ),
                    "max_s": len(input_ids),
                    "lm_head_indices": torch.tensor(
                        [0 + len(input_ids) - 1], dtype=torch.int32, device=device
                    ),
                }
            else:
                # incremental step
                ...
                inputs = {
                    ...
                    "position_ids": torch.tensor(
                        [current_length - 1], dtype=torch.int32, device=device
                    ),
                    ...
                    "input_lengths": torch.tensor(
                        [current_length], dtype=torch.int32, device=device
                    ),
        ...
        print(
            f"total new tokens: {max_new_tokens}, cost time: {sum(times):.6f} s\n"
